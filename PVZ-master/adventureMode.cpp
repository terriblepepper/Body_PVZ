...
void adventureGameMode::startGame() {
	...
	if (GestureControl)
	{
        ...
        m_gestureMouseThread = new GestureMouseThread(this);
        if (view) {
            m_gestureMouseThread->setGameView(view);
        }
        else {
            qWarning() << "Game view (QGraphicsView) is null, cannot set for gesture confinement.";
        }
        m_gestureMouseThread->start();
	}
}
...
adventureGameMode::~adventureGameMode()
{
    ...
    {    
        ...
        if (m_gestureMouseThread)
        {
            m_gestureMouseThread->stop();
            m_gestureMouseThread->wait(); // 等待线程安全退出
            delete m_gestureMouseThread;
            m_gestureMouseThread = nullptr;
        }
    }
...
void adventureGameMode::keyPressEvent(QKeyEvent* event)
{
    if (event->key() == Qt::Key_F1) {
        qDebug() << "F1 key pressed in adventureGameMode";
        if (GestureControl && m_gestureMouseThread) { // 确保手势控制启用且线程对象存在
            m_gestureMouseThread->togglePause();
        }
        event->accept(); // 表示事件已处理
    }
    else {
        // 如果不是F1键，调用基类的实现，以便其他按键（如Esc可能关闭窗口等）能正常工作
