...
int puzzleMode::level = 0;
bool puzzleMode::willDelete;
puzzleMode::puzzleMode(QWidget* parent) : QWidget(parent) {
    setupUi();
    ...
    if (GestureControl)
    {
        gestureReceiver = new HandGestureReceiver();
    }
}

void puzzleMode::goToGamingMenu()
{
...
void puzzleMode::setupUi() {
    puzzleMode::willDelete = false;
    ...
    back->setStyleSheet("QPushButton {"
        ...
        "}");
    connect(this->back, &QPushButton::clicked, this, &puzzleMode::onBackClicked);
    ...
    for (int i = 1; i <= 20; ++i) {
        ...
        levelButton->setText("打僵尸：" + QString("关卡%1").arg(i));
        //设置button的工程名字
        ...
        connect(levelButton, &QPushButton::clicked, [this, levelButton]() {
            ...
            puzzleMode::level = temp.remove("level_").toInt();//按下关卡按钮就立马设置关卡
            emit stopLoadingBGM();
            startGame();
            });
...
void puzzleMode::loadCards(const QString& filename)
{
...
void puzzleMode::startGame() {
    ...
    mQTimer->setInterval(33333 / fpsIndex);
    // 创建游戏场景并设置其分辨率
    ...
    Hammer->setPos(578, 252); // 确保锤子初始位置在可视范围内
    scene->addItem(Hammer);
    ...
    connect(mQTimer, &TimerThread::timeout, this, &puzzleMode::addZombie);
    // 连接高精度定时器的timeout信号到游戏的check槽，检查游戏是否结束
    connect(mQTimer, &TimerThread::timeout, this, &puzzleMode::checkGameState);
    //添加菜单按钮
    createMenuButton();
    //连接到菜单按钮
    connect(menuButton, &QPushButton::clicked, this, &puzzleMode::goToGamingMenu);
    //连接手势控制
	if (GestureControl)
	{
		mQTimer2 = new QTimer(this);
        mQTimer2->setInterval(10);
		connect(mQTimer2, &QTimer::timeout, [this, Hammer]()
			{
                if (globalGestureState.KeyPointState == "hammer")
                {
                    Hammer->setHammerState(globalGestureState.HistoryPointState,2.0,false);
                    ismouseControl = false;
                }
                if (globalGestureState.KeyPointState == "idle" && !ismouseControl)
                {
                    Hammer->setHammerState(" ",0.0,false);
                    ismouseControl = true;
                }
			});
        mQTimer2->start();
		connect(mQTimer, &TimerThread::timeout, [this, Hammer]()
			{
				if(!(globalGestureState.KeyPointState == "hammer"||globalGestureState.KeyPointState == "idle"))Hammer->setHammerPosition(globalGestureState.x * 910 + 180., globalGestureState.y * 610.);
			});
	}
    view->show();
    //播放BGM
...
void puzzleMode::checkGameState()
{
    ...
    //坚持一定时间就胜利
    if ((double)timeCounter / (double)(30 * fpsIndex) > 90. * (1. + (double)puzzleMode::level * 0.45))
    {
        winScene();
    }
}
//待修改
void puzzleMode::addZombie()
{
    if (++counter >= time)
    {
        low++;
...
void puzzleMode::createMenuButton()
{
...
void puzzleMode::failScene()
{
    ...
    connect(gameOverButton, &QPushButton::clicked, this, &puzzleMode::gameFinish);
}

void puzzleMode::winScene()
{
    // 结束计时器停止游戏
    mQTimer->stop();
    // 添加游戏胜利的结束图片
    ...
    gamingWidgetsProxy->setPos(0, 0);
    connect(gameWinButton, &QPushButton::clicked, this, &puzzleMode::gameFinish);
}
...
puzzleMode::~puzzleMode()
{
    ...
    if (GestureControl)
    {
        ...
        if (gestureReceiver)
        {
            delete gestureReceiver;
            gestureReceiver = nullptr;
        }
        if (mQTimer2)
        {
            mQTimer2->stop();
...
void puzzleMode::getGamingMenu(gamingMenuDialog* gM) {
    gamingMenu = gM;
}
