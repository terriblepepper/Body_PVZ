    ...
    : QWidget{parent}
{
    ...
    connect(btn_puzzlegame, &QPushButton::clicked, [this]() {
        gamingMenu = new(gamingMenuDialog);//创建游戏菜单
        puzzleGaming = new (puzzleMode);//创建游戏窗口
...
void startpage::updateSettings(int volumeBGM,int volumeItem, const QString& difficulty, int fps, bool gesture)
{
...
void startpage::writeSettingsToFile(const QString& filePath)
{
	...
	settings.setValue("GestureControl", GestureControl);
    settings.endGroup();
}
